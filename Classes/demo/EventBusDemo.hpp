...
*/
class EventBusDemo : public cocos2d::Ref
{
public:
	EventBusDemo() {
	...
	*/
	void Demo1() {
		...
		while (x <= 1000) {
			CCLOG("Changing player 1 X to %d\n", x);
			...
			if (setPlayerPostionWithEvent(player1, x, 0, 0) == true) {
				...
				CCLOG("Setting player 1 position was canceled\n");
				break;
			}
		}
		...
		while (x <= 1000) {
			CCLOG("Changing player 2 X to %d\n", x);
			if (setPlayerPostionWithEvent(player2, x, 0, 0) == true) {
				x += 200;
			}
			else {
				CCLOG("Setting player 2 position was canceled\n");
				break;
			}
		}
...
inline void eventDemo()
{
	CCLOG("* * * EventBus Demo Program * * * \n");
